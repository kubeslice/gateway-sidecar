...
import (
	...
	"bitbucket.org/realtimeai/mesh-netops/sidecar/metrics"
	"github.com/go-ping/ping"
	"github.com/pkg/errors"

	"bitbucket.org/realtimeai/mesh-netops/sidecar/cmd"
...
// Execute executes the Tunnel status check.By
...
func (t *TunnelChecker) UpdatePeerIP(peerIP string) {
...
func (t *TunnelChecker) updateNetworkStatus(ifaceName string) error {
	...
	// for _, ioStat := range netIoStats {
	// 	if ioStat.IntName == ifaceInfos[0].Name {
	// 		txBytes = uint64(ioStat.TX)
	// 		rxBytes = uint64(ioStat.RX)
	// 		t.log.Infof("Tx: %v Rx : %v I Pack: %v O Pack : %v I Err: %v O Err : %v ", txBytes, rxBytes, ioStat.IPackets, ioStat.OPackets, ioStat.IErrors, ioStat.OErrors)
	// 		break
	// 	}
	// }
	...
	if err != nil {
		...
		rxBytes, err = strconv.ParseUint(strings.TrimSuffix(cmdOut, "\n"), 10, 64)
		if err != nil {
			// t.log.Errorf("rxBytes integer conversion failed %v", err)
		}
	}
