...
if [ ! $(kind get clusters | grep controller) ];then
  ...
  kind load docker-image gw-sidecar:${GITHUB_HEAD_COMMIT} --name controller
...
if [ ! $(kind get clusters | grep worker) ];then
  ...
  # loading docker image into kind netops
  kind load docker-image gw-sidecar:${GITHUB_HEAD_COMMIT} --name worker
...
WorkerChartOptions:
  ...
  SetStrValues:
    ...
    "gateway.tag": "${GITHUB_HEAD_COMMIT}"
